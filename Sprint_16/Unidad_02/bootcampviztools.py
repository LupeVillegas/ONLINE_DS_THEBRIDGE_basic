...
#set names of images as y
def read_data(directorio, reshape_dim ):
    X = []
    y = []
...
def show_images(imagenes, labels=[], n_cols=6, size_scale=3):
...
def misclassified_images(y_test, y_pred_class, y_pred_proba, mis_indx):
    ...
    misclassified = np.array(misclassified, dtype=object)
    
    
    selected_indx = []

    for cls in np.unique(y_test):
        # Filter misclassified samples where true class = cls
        cls_errors = misclassified[misclassified[:,1] == cls]
    
        if len(cls_errors) > 0:
            # Sort by confidence (descending)
            cls_errors = cls_errors[cls_errors[:,3].argsort()[::-1]]
        
            # Take top 10% (at least 1 sample)
            k = max(1, int(0.1 * len(cls_errors)))
            selected_indx.extend(cls_errors[:k,0].astype(int))

    selected_indx = np.array(selected_indx)
    
    return selected_indx
    

def plot_misclassified(X_test, y_test, selected_indx, y_pred_proba,y_pred_class,images):

    n = len(selected_indx)
    plt.figure(figsize=(12, 12))
    for j, i in enumerate(selected_indx[:images]): 
        if y_pred_class[i] == 0:
            pred = 'cat'
        else:
            pred = 'dog'
        ...
        plt.title(f"T:{y_test[i]} P:{pred} Conf:{y_pred_proba[i][y_pred_class[i]]:.1f}")
        plt.axis("off")

    plt.tight_layout()
    plt.show()

#set y as the folders names
def read_data_f(directorio, reshape_dim ):
    X = []
    y = []
    for folder in os.listdir(directorio):
        if os.path.isdir('/'.join([directorio, folder])):
